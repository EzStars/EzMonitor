...
- `LCP`：最大内容渲染时间，衡量主要内容何时出现在屏幕上
- `CLS`：累计布局偏移，衡量页面视觉稳定性
- `INP`：交互到下一次绘制的时间，衡量交互响应延迟
...
95 分位不是平均值，而是把一批数据从小到大排序后，位于 95% 位置附近的那个值。它的含义是：95% 的样本都不超过这个值，剩下 5% 的慢样本会更高。

在性能监控里，95 分位比平均值更适合做告警，因为它更容易暴露“少数很慢，但平均值看不出来”的尾部问题。
...
- `FP` / `FCP`：观察到第一个最终值后即可上报，属于一次性指标。
- `TTFB`：可以在 `navigation` entry 出现后直接采集。
- `LCP`：需要 buffered 观察，等待页面生命周期结束时再提交最终值。
- `CLS`：不能只看单个 shift，应该按 session window 累积，页面隐藏或离开时上报最终值。
- `INP`：优先在支持 `event` timing 的浏览器中采集，最终值也建议在页面离开时统一提交。
