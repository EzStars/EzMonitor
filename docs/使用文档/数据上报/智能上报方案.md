...
`monitor-sdkv2` 当前上报通道包含：

- `tracking`：行为事件（含 `page_view` / `uv_visit`）
- `performance`：性能指标
- `error`：错误日志
- `replay`：回放分段（segment）

其中 `replay` 采用独立通道，便于后端单独存储与查询，不与 tracking 混合。

### 推荐策略

- 对 replay 采用分段上报（控制单次 payload 大小）
- 页面隐藏、离开、错误发生时优先 flush replay
- 大分段只上报最近样本（尾部截断），避免单包过大失败
 ...
 * 建立 SSE 连接
 */
export async function connectSSE(ctx: Context) {
